...
namespace AirQualityServer.Workers
{
    public sealed class IQAirApiClient
    {
        ...
        public async Task<CityData> FetchCityDataAsync(string city, string state, string country, string requestId)
        {
            ...
            Logger.Info($"Calling IQAir API: city={city}, state={state}, country={country}", requestId);
            ...
            {
                Logger.Warning($"IQAir API returned HTTP {(int)response.StatusCode}: {body}", requestId);
            ...
            Logger.Info($"IQAir API success: AQI={parsed.Data.Current?.Pollution?.AqiUs}", requestId);
            return parsed.Data;
        }
    }
}
